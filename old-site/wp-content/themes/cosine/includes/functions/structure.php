...
if ( ! function_exists( 'cosine_head_title' ) ) {
	add_filter( 'wp_title', 'cosine_head_title', 10, 2 );
	 ...
	 * @return  string
	 */
	function cosine_head_title( $title, $sep ) {
		...
		if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
			$title = "$title $sep " . sprintf( esc_html__( 'Page %s', 'cosine' ), max( $paged, $page ) );
		}
...
if ( ! function_exists( 'cosine_body_classes' ) ) {
	add_filter( 'body_class', 'cosine_body_classes' );

	function cosine_body_classes( $classes ) {
...
if ( ! function_exists( 'cosine_post_classes' ) ) {
	add_filter( 'post_class', 'cosine_post_classes' );

	function cosine_post_classes( $classes ) {
...
if ( ! function_exists( 'cosine_override_page_menu' ) ) {
	add_filter( 'wp_nav_menu_args', 'cosine_override_page_menu' );
	 ...
	 * @return  array
	 */
	function cosine_override_page_menu( $params ) {
		if ( is_page() ) {
...
if ( ! function_exists( 'cosine_extra_menu_items' ) ) {
	add_action( 'theme/after_primary_menu', 'cosine_extra_menu_items' );
	 ...
	 * @return  void
	 */
	function cosine_extra_menu_items( $args ) {
		if ( $args['env'] == 'desktop' ) {
...
if ( ! function_exists( 'cosine_fix_post_title' ) ) {
	add_filter( 'the_title', 'cosine_fix_post_title', 10, 2 );
	 ...
	 * @return  string
	 */
	function cosine_fix_post_title( $title, $id ) {
		$post = get_post( $id );
